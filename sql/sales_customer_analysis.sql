...
SELECT TOP 5
	cust.cust_id,
	ROUND(SUM(product_quantity * product_price), 2) AS 'Total Spending in 2019 (in USD)'
FROM [FictionalSalesDatabase].[dbo].[FactSalesTransactions] sales
...
( 
	...
	(
		...
		FROM 
		(
			SELECT TOP 50 PERCENT cust_age
			FROM [FictionalSalesDatabase].[dbo].[DimCustomer]
			ORDER BY 1
		) a
	)
	...
	) / 2 AS median  --> The median value of customer age is 66 years old
)

SELECT
	MIN(cust_age) AS 'Min Age of Customer',
	(SELECT median FROM cte_median) AS 'Median Age of Customer',
	AVG(cust_age) AS 'Avg Age of Customer',
	MAX(cust_age) AS 'Max Age of Customer',
	MAX(cust_age) - MIN(cust_age) AS 'Range'
FROM [FictionalSalesDatabase].[dbo].[DimCustomer];

-- Customer Demographic (City) Distribution
SELECT 
	cust_city,
	COUNT(cust_id) AS 'Number of Unique Customer'
FROM [FictionalSalesDatabase].[dbo].[DimCustomer]
GROUP BY cust_city
ORDER BY 2 DESC;
...
SELECT 
	cust_city,
	ROUND(SUM(sales.product_quantity * prod.product_price), 2) AS 'Total Spending (in USD)'
FROM [FictionalSalesDatabase].[dbo].[DimCustomer] cust
JOIN [FictionalSalesDatabase].[dbo].[FactSalesTransactions] sales 
